...
# Import encryption functionality - FIX: Use absolute import path
from KNN.knn_encryption import (
    secure_encrypt_vector, secure_decrypt_vector,
    encrypt_document_aes, decrypt_document_aes,
    encrypt_doc_id, decrypt_doc_id
)
...
collection_name = "V2_Encrypted_Data_March_2025"
client = MilvusClient(
    uri=ZILLIZ_ENDPOINT,
    token=ZILLIZ_TOKEN
)
...
def get_embeddings(text):
    ...
    time.sleep(0.5)
    # Encrypt the embedding before returning
    encrypted_embedding = secure_encrypt_vector(np.array(response.embeddings[0]))
...
def get_query_embedding(query):
    ...
    # Encrypt the query embedding to match encrypted vectors in the database
    encrypted_embedding = secure_encrypt_vector(np.array(response.embeddings[0]))
    return encrypted_embedding
...
def format_search_results(search_results):
    ...
    for i, hit in enumerate(search_results[0], 1):
        ...
        encrypted_chunk_text = getattr(entity, 'chunk_text', 'No text available')
        source_url = getattr(entity, 'source_url', 'N/A')
        upload_date = getattr(entity, 'upload_date', 'N/A')
        encrypted_id = getattr(entity, 'id', None)
        
        # Decrypt the chunk text
        if encrypted_chunk_text != 'No text available':
            try:
                chunk_text = decrypt_document_aes(encrypted_chunk_text)
            except Exception:
                chunk_text = f"[Error decrypting text: {encrypted_chunk_text[:20]}...]"
        else:
            chunk_text = encrypted_chunk_text
        
        # Safely handle ID decryption
        if encrypted_id is not None:
            # Check if ID is in the expected string format
            if isinstance(encrypted_id, str):
                try:
                    doc_id = decrypt_doc_id(encrypted_id)
                except Exception:
                    doc_id = f"[Encrypted ID: {encrypted_id[:10]}...]"
            else:
                # If it's an auto-generated ID or other type
                doc_id = f"System ID: {encrypted_id}"
        else:
            doc_id = "Unknown ID"
        ...
        formatted.append(f"""
Result {i} (Similarity Score: {1 - hit.distance:.2f}, ID: {doc_id}):
------------------------------------------------------------------
...
def perform_single_vector_search(query_embedding):
    search_results = collection.search(
        ...
        limit=5,
        output_fields=["chunk_text", "source_url", "upload_date", "id"]  # Explicitly include "id"
    )
...
def process_data(data, source_url):
    ...
    for i, chunk_text in enumerate(chunks):
        ...
        # Encrypt the chunk text content
        encrypted_chunk_text = encrypt_document_aes(chunk_text)
        
        # Generate a unique ID and encrypt it
        chunk_id = int(time.time() * 1000) + i
        encrypted_id = encrypt_doc_id(chunk_id)
        
        chunk_dict = {
            "id": encrypted_id,  # Store encrypted ID
            "source_url": source_url,
            "chunk_text": encrypted_chunk_text,  # Store encrypted text
            "upload_date": time.strftime("%Y-%m-%d %H:%M:%S"),
            "vector": embedding  # Already encrypted by get_embeddings
        }
